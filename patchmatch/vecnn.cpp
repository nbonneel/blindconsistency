...
VECBITMAP<unsigned char> *bitmap_to_patches(Params *p, PATCHBITMAP *a) {
  //int n = -1, d = -1;
...
VECBITMAP<int> *bitmap_to_vecbitmap(PATCHBITMAP *a) {
...
PATCHBITMAP *vecwrap_init_nn(int vec_mode, Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask, PATCHBITMAP **ann_sim) {
  if (vec_mode == VEC_MODE_PATCH) {
    ...
    PATCHBITMAP *ans = vec_init_nn<unsigned char>(p, av, bv, bmask, region_masks, amask);
    delete av;
    delete bv;
    return ans;
  } 
...
PATCHBITMAP *vecwrap_init_dist(int vec_mode, Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *ann, PATCHBITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask, PATCHBITMAP *ann_sim) {
  if (vec_mode == VEC_MODE_PATCH) {
    ...
    delete av;
    delete bv;
    PATCHBITMAP *ansd = vecbitmap_to_bitmap(ans);
    delete ans;
    return ansd;
  } 
	else if (vec_mode == VEC_MODE_SIM) {
...
void vecwrap_nn(int vec_mode, Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
        ...
        RegionMasks *region_masks, int tiles, PATCHBITMAP *ann_sim) 
{
  if (vec_mode == VEC_MODE_PATCH) {
    ...
    PATCHBITMAP *anndp = vecbitmap_to_bitmap(anndv);
...
PATCHBITMAP *vecwrap_vote(int vec_mode, Params *p, PATCHBITMAP *b,
         PATCHBITMAP *ann, PATCHBITMAP *ann_sim, PATCHBITMAP *bnn,
