...
PATCHBITMAP *norm_image(double *accum, int w, int h) {
...
PATCHBITMAP *norm_image(int *accum, int w, int h) {
  PATCHBITMAP *ans = create_bitmap(w, h);
#pragma omp parallel for schedule(static, 16)
  for (int y = 0; y < h; y++) {
    int *row = (int *) ans->line[y];
...
PATCHBITMAP *norm_image_init(int *accum, int w, int h, PATCHBITMAP *ainit) {
  PATCHBITMAP *ans = create_bitmap(w, h);
#pragma omp parallel for schedule(static, 16)
  for (int y = 0; y < h; y++) {
    int *row = (int *) ans->line[y];
...
PATCHBITMAP *norm_image_init(double *accum, int w, int h, PATCHBITMAP *ainit) {
  PATCHBITMAP *ans = create_bitmap(w, h);
#pragma omp parallel for schedule(static, 16)
  for (int y = 0; y < h; y++) {
    int *row = (int *) ans->line[y];
...
PATCHBITMAP *init_dist_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *ann, PATCHBITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask) {
...
PATCHBITMAP *vote_n(Params *p, PATCHBITMAP *b,
							 PATCHBITMAP *ann, PATCHBITMAP *bnn,
							 PATCHBITMAP *bmask, PATCHBITMAP *bweight,
							 double coherence_weight, double complete_weight,         
							 RegionMasks *amask, PATCHBITMAP *aweight, PATCHBITMAP *ainit, RegionMasks *region_masks, PATCHBITMAP *aconstraint, int mask_self_only) 
{
...
template<int PATCH_W>
PATCHBITMAP *vote_n_openmp(Params *p, PATCHBITMAP *b,
											PATCHBITMAP *ann, PATCHBITMAP *bnn,
                      PATCHBITMAP *bmask, PATCHBITMAP *bweight,
                      double coherence_weight, double complete_weight,
                      RegionMasks *amask, PATCHBITMAP *aweight, PATCHBITMAP *ainit, RegionMasks *region_masks, PATCHBITMAP *aconstraint, int mask_self_only) {
...
PATCHBITMAP *vote_n_cputiled(Params *p, PATCHBITMAP *b,
                        PATCHBITMAP *ann, PATCHBITMAP *bnn,
                        PATCHBITMAP *bmask, PATCHBITMAP *bweight,
                        double coherence_weight, double complete_weight,
                        RegionMasks *amask, PATCHBITMAP *aweight, PATCHBITMAP *ainit, RegionMasks *region_masks, PATCHBITMAP *aconstraint, int mask_self_only) 
...
int window_constraint(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, int ax, int ay, int bx, int by, PATCHBITMAP *ann_window, PATCHBITMAP *awinsize) {
...
template<int PATCH_W, int IS_MASK, int IS_WINDOW>
void nn_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
          PATCHBITMAP *ann, PATCHBITMAP *annd,
          ...
          int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, RegionMasks *region_masks, int tiles,
					PATCHBITMAP *ann_window, PATCHBITMAP *awinsize) 
{
...
template<int PATCH_W, int IS_MASK, int IS_WINDOW>
void nn_n_gpucpu(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
          PATCHBITMAP *ann, PATCHBITMAP *annd,
          RegionMasks *amask, PATCHBITMAP *bmask,
          int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, RegionMasks *region_masks, int tiles) {
  ...
  Box box = get_abox(p, a, amask);
  PATCHBITMAP *ann0 = ann, *annd0 = annd;
  PATCHBITMAP *ann_out = create_bitmap(ann->w, ann->h);
  PATCHBITMAP *annd_out = create_bitmap(annd->w, annd->h);
  PATCHBITMAP *ann_out0 = ann_out, *annd_out0 = annd_out;
  PATCHBITMAP *last_iter = NULL;
...
/* for multi-core */
template<int PATCH_W, int IS_MASK, int IS_WINDOW>
void nn_n_cputiled(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
          PATCHBITMAP *ann, PATCHBITMAP *annd,
          RegionMasks *amask, PATCHBITMAP *bmask,
          int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, RegionMasks *region_masks, int tiles,		  
					PATCHBITMAP *ann_window, PATCHBITMAP *awinsize) 
...
template<int PATCH_W>
void nn_n_proponly(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
          PATCHBITMAP *ann, PATCHBITMAP *annd,
          RegionMasks *amask, PATCHBITMAP *bmask,
          int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, RegionMasks *region_masks, int tiles) {
  if (tiles < 0) { tiles = p->cores; }
...
template<int PATCH_W, int IS_MASK, int IS_WINDOW>
void nn_n_fullrand(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
          PATCHBITMAP *ann, PATCHBITMAP *annd,
          RegionMasks *amask, PATCHBITMAP *bmask,
          int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, RegionMasks *region_masks, int tiles) {
...
RegionMasks::RegionMasks(Params *p, PATCHBITMAP *region_masks, int full, PATCHBITMAP *bmask) {
  bmp = region_masks;
...
Box get_abox(Params *p, PATCHBITMAP *a, RegionMasks *amask, int trim_patch) {
  if (!amask) {
    Box ans;
    ans.xmin = ans.ymin = 0;
...
/* TODO: We can use a PATCHBITMAP * for region_masks now, no need to calculate the bboxes in RegionMasks constructor. */
/* initialize the NN field */
PATCHBITMAP *init_nn(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask, int trim_patch, PATCHBITMAP *ann_window, PATCHBITMAP *awinsize) {
  PATCHBITMAP *bmp = create_bitmap(a->w, a->h);
  clear(bmp);
...
PATCHBITMAP *init_dist(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *ann, PATCHBITMAP *bmask, RegionMasks *region_masks, RegionMasks *amask) {
  PATCHBITMAP *ans = NULL;
...
void nn(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
        PATCHBITMAP *ann, PATCHBITMAP *annd,
        RegionMasks *amask, PATCHBITMAP *bmask,
        int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, int cache_b,
        RegionMasks *region_masks, int tiles, PATCHBITMAP *ann_window, PATCHBITMAP *awinsize) {
  int algo = p->algo;
  if (algo == ALGO_CPU) {
    if (is_window(p)) {
...
PATCHBITMAP *vote(Params *p, PATCHBITMAP *b,
          PATCHBITMAP *ann, PATCHBITMAP *bnn,
          PATCHBITMAP *bmask, PATCHBITMAP *bweight,
          double coherence_weight, double complete_weight,
          RegionMasks *amask, PATCHBITMAP *aweight, PATCHBITMAP *ainit, RegionMasks *region_masks, PATCHBITMAP *aconstraint, int mask_self_only) {
...
int patch_dist(Params *p, PATCHBITMAP *a, int ax, int ay,
               PATCHBITMAP *b, int bx, int by, int maxval, RegionMasks *region_masks) {
...
void check_colors(PATCHBITMAP *bmp) {
...
int clip_nn(Params *p, PATCHBITMAP *ann, PATCHBITMAP *b) {
  int ans = 0;
...
void check_nn(Params *p, PATCHBITMAP *ann, PATCHBITMAP *b, PATCHBITMAP *bmask, RegionMasks *amask, RegionMasks *region_masks) {
  Box box = get_abox(p, ann, amask);
  for (int y = box.ymin; y < box.ymax; y++) {
    for (int x = box.xmin; x < box.xmax; x++) {
...
void check_dists(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *ann, PATCHBITMAP *annd, int max_dist, RegionMasks *amask) {
  check_colors(a);
  check_colors(b);
  Box box = get_abox(p, a, amask);
...
PATCHBITMAP *copy_image(PATCHBITMAP *a) {
...
void getnn(PATCHBITMAP *ann, int x, int y, int &xp, int &yp) {
...
void minnn_n(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *ann, PATCHBITMAP *annd, PATCHBITMAP *ann_prev, PATCHBITMAP *bmask, int level, int em_iter, RecomposeParams *rp, RegionMasks *region_masks, RegionMasks *amask, int ntiles) {
...
void minnn(Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *ann, PATCHBITMAP *annd, PATCHBITMAP *ann_prev, PATCHBITMAP *bmask, int level, int em_iter, RecomposeParams *rp, RegionMasks *region_masks, RegionMasks *amask, int ntiles) {
  if (is_window(p)) {
