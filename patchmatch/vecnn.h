...
template<class T>
inline PATCHBITMAP *wrap_vecbitmap(VECBITMAP<T> *a) {
	return new PATCHBITMAP(a->w, a->h); // no memory allocated for per pixel data
}

// nothing special here, just re-use init_nn to assign a random NN field
template<class T>
PATCHBITMAP *vec_init_nn(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *bmask=NULL, RegionMasks *region_masks=NULL, RegionMasks *amask=NULL) {
	PATCHBITMAP *awrap = wrap_vecbitmap(a);
	PATCHBITMAP *bwrap = wrap_vecbitmap(b);
	PATCHBITMAP *ans = init_nn(p, awrap, bwrap, bmask, region_masks, amask, 0);
  delete awrap;
  delete bwrap;
  return ans;
}
...
Box get_abox_vec(Params *p, VECBITMAP<T> *a, RegionMasks *amask, int trim_patch=1) {
  PATCHBITMAP *aw = wrap_vecbitmap(a);
...
VECBITMAP<ACCUM> *vec_init_dist_n(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *ann, PATCHBITMAP *bmask=NULL, RegionMasks *region_masks=NULL, RegionMasks *amask=NULL) {
...
VECBITMAP<ACCUM> *vec_init_dist(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *ann, PATCHBITMAP *bmask=NULL, RegionMasks *region_masks=NULL, RegionMasks *amask=NULL) {
...
void vec_nn_n(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b,
            PATCHBITMAP *ann, VECBITMAP<ACCUM> *annd,
...
void nn(Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
        ...
        RegionMasks *amask=NULL, PATCHBITMAP *bmask=NULL,
        int level=0, int em_iter=0, RecomposeParams *rp=NULL, int offset_iter=0, int update_type=0, int cache_b=0,
        RegionMasks *region_masks=NULL, int tiles=-1, PATCHBITMAP *ann_window=NULL, PATCHBITMAP *awinsize=NULL);
*/
// XC -->
...
void vec_minnn_n(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *ann, VECBITMAP<ACCUM> *annd, PATCHBITMAP *ann_prev, PATCHBITMAP *bmask, int level, int em_iter, RecomposeParams *rp, RegionMasks *region_masks, RegionMasks *amask, int ntiles) {
...
template<class T, class ACCUM>
void vec_minnn(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *ann, VECBITMAP<ACCUM> *annd, PATCHBITMAP *ann_prev, PATCHBITMAP *bmask, int level, int em_iter, RecomposeParams *rp, RegionMasks *region_masks, RegionMasks *amask, int ntiles) {
  if (is_window(p))
...
template<class T>
PATCHBITMAP *XCvec_init_nn(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *bmask=NULL, RegionMasks *region_masks=NULL, RegionMasks *amask=NULL) {
  PATCHBITMAP *awrap = wrap_vecbitmap(a);
  PATCHBITMAP *bwrap = wrap_vecbitmap(b);
  PATCHBITMAP *ans = init_nn(p, awrap, bwrap, bmask, region_masks, amask, 1); // trim patch!!!
  delete awrap;
  delete bwrap;
  return ans;
}
...
VECBITMAP<ACCUM> *XCvec_init_dist_n(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *ann, PATCHBITMAP *bmask=NULL, RegionMasks *region_masks=NULL, RegionMasks *amask=NULL) \
{
	VECBITMAP<ACCUM> *ans = new VECBITMAP<ACCUM>(a->w, a->h, 1); // the third dimension only has one element
...
template<class T, class ACCUM>
VECBITMAP<ACCUM> *XCvec_init_dist(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *ann, PATCHBITMAP *bmask=NULL, RegionMasks *region_masks=NULL, RegionMasks *amask=NULL) {
	VECBITMAP<ACCUM> *ans = NULL;
	if (is_window(p)) { 
...
void XCvec_minnn_n(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *ann, VECBITMAP<ACCUM> *annd, PATCHBITMAP *ann_prev, PATCHBITMAP *bmask, int level, int em_iter, RecomposeParams *rp, RegionMasks *region_masks, RegionMasks *amask, int ntiles) {
  if (ntiles < 0) { ntiles = p->cores; }
  printf("vec_minnn: %d %d %d %d, tiles=%d\n", ann->w, ann->h, ann_prev->w, ann_prev->h, ntiles);
  if (!rp) { fprintf(stderr, "vec_minnn_n: rp is NULL\n"); exit(1); }
//  double start_t = accurate_timer();
...
template<class T, class ACCUM>
void XCvec_minnn(Params *p, VECBITMAP<T> *a, VECBITMAP<T> *b, PATCHBITMAP *ann, VECBITMAP<ACCUM> *annd, PATCHBITMAP *ann_prev, PATCHBITMAP *bmask, int level, int em_iter, RecomposeParams *rp, RegionMasks *region_masks, RegionMasks *amask, int ntiles) {
	if (is_window(p)) {
...
PATCHBITMAP *vecbitmap_to_bitmap(VECBITMAP<int> *a);

#define VEC_MODE_PATCH 0
#define VEC_MODE_DESC  1
#define VEC_MODE_SIM   2

PATCHBITMAP *vecwrap_init_nn(int vec_mode, Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *bmask=NULL, RegionMasks *region_masks=NULL, RegionMasks *amask=NULL, PATCHBITMAP **ann_sim=NULL);
PATCHBITMAP *vecwrap_init_dist(int vec_mode, Params *p, PATCHBITMAP *a, PATCHBITMAP *b, PATCHBITMAP *ann, PATCHBITMAP *bmask=NULL, RegionMasks *region_masks=NULL, RegionMasks *amask=NULL, PATCHBITMAP *ann_sim=NULL);
void vecwrap_nn(int vec_mode, Params *p, PATCHBITMAP *a, PATCHBITMAP *b,
        PATCHBITMAP *ann, PATCHBITMAP *annd,
        RegionMasks *amask=NULL, PATCHBITMAP *bmask=NULL,
        int level=0, int em_iter=0, RecomposeParams *rp=NULL, int offset_iter=0, int update_type=0, int cache_b=0,
        RegionMasks *region_masks=NULL, int tiles=-1, PATCHBITMAP *ann_sim=NULL);
				PATCHBITMAP *vecwrap_vote(int vec_mode, Params *p, PATCHBITMAP *b,
				PATCHBITMAP *ann, PATCHBITMAP *ann_sim=NULL, PATCHBITMAP *bnn=NULL,
